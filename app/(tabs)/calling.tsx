...
export default function CallingScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      queryClient.invalidateQueries({ queryKey: ['group/get-groups'] });
      fetchGroups().catch(() => {
        Alert.alert('Error', 'Failed to fetch groups. Please try again.');
      });
    }, [queryClient, fetchGroups, stableUserId])
  );
    ...
    return (
      <SafeAreaView className="flex-1 bg-gradient-to-b from-blue-50 to-white" edges={['top']}>
        ...
        <View className="flex-1 p-5">
            ...
            <ScrollView>
              {currentCall.contacts.map((contact, index) => {
                ...
                return (
                  <View
                    key={contact.id}
                    className="flex-row items-center p-3 mb-2 bg-white rounded-lg shadow-sm"
                  >
          ...
          <TouchableOpacity
            className="items-center py-3 mt-4 rounded-lg bg-error"
